var utils = (function() {
    ...
    // Fill the cache.
    // Only for hierarchical relationship do we ignore the user selection and look at
    // the entire 'model' to accurately determine common ancestors and element depth.
    ...
    // While we need to consider the entire 'model' when looking upwards, we do not
    // look down the hierarchy. As a side effect, constraints that exempt leaf-level
    // elements may be applied too loosely when the user does not include lower-level
    // views in his selection.
